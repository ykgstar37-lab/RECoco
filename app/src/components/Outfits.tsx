...
const FIT: Record<OutfitId, { from: [number, number]; to: [number, number]; k: number; r?: number; top: number }> = {
  ...
  dog: { from: [0, 0], to: [0, 0], k: 1, top: 94 },
...
function Hat({ id }: { id: OutfitId }) {
  switch (id) {
    ...
    case 'dog':
      // 머리 양옆에 늘어진 검은 강아지 귀 (붙는 모서리가 머리 테두리 위로 물리게 얹는다)
      return (
        <G fill={EAR}>
          <Path d="M114,98 C82,96 48,116 40,152 C33,182 42,212 62,218 C81,223 96,206 102,182 C109,158 113,122 114,98 Z" />
          <Path d="M286,98 C318,96 352,116 360,152 C367,182 358,212 338,218 C319,223 304,206 298,182 C291,158 287,122 286,98 Z" />
        </G>
      );
    case 'straw':
      return (
