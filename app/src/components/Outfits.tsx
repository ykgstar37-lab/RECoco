...
const FIT: Record<OutfitId, { from: [number, number]; to: [number, number]; k: number; r?: number; top: number }> = {
  ...
  dog: { from: [0, 0], to: [0, 0], k: 1, top: 82 },
...
function Hat({ id }: { id: OutfitId }) {
  switch (id) {
    ...
    case 'dog':
      // 머리 양옆에 늘어진 검은 강아지 귀. 윗선이 머리 테두리를 따라가야 사이에 배경이 안 비친다
      return (
        <G fill={EAR}>
          <Path d="M140,85 C110,102 84,126 66,152 C44,176 16,192 14,216 C12,240 32,250 54,242 C74,234 86,214 92,188 C100,152 117,113 140,85 Z" />
          <Path d="M260,85 C290,102 316,126 334,152 C356,176 384,192 386,216 C388,240 368,250 346,242 C326,234 314,214 308,188 C300,152 283,113 260,85 Z" />
        </G>
      );
    case 'straw':
      return (
